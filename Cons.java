            ...
            public void map(String key, String value, MapReduce mr) {
                ...
                for (int i = 0; i < value.length(); i++)
                {
                  ...
                  if (Character.isUpperCase(a))
                  {
                    ...
                    String b = ((Character) a).toString();
                    mr.collect_map(b, list("1"));
                  }
                  else if (Character.isLowerCase(a))
                  {
                    String c = ((Character) a).toString();
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                StringTokenizer str = new StringTokenizer(value, " !,.?:;'-\t", false);
                while (str.hasMoreTokens())
                {
                  String a = str.nextToken();
                  String fin = a.toLowerCase();
                  mr.collect_map(fin, list("1"));
                }
            } };
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                int sublength = value.length() - grepkey.length();
                Integer k = Integer.decode(key);
                for (int i = 0; i < sublength; i++)
                {
                  if (value.charAt(i) == grepkey.charAt(0))
                  {
                    String sub = value.substring(i, i + grepkey.length());
                    if (grepkey.compareTo(sub) == 0)
                    {
                      mr.collect_map(grepkey, list(k));
                    }
                  }
                }
            } };
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                while (str.hasMoreTokens())
                {
                  String movNum = str.nextToken();
                  String userNum = str.nextToken();
                  String movRat = str.nextToken();
                  String movDat = str.nextToken();
                  mr.collect_map(movNum, list(movRat));
                }
            } };
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                while (value != null)
                {
                  sum += Integer.decode((String)first((Cons)first(value)));
                  value = rest(value);
                }
                double average = sum / num;
                mr.collect_reduce(key, list(average, num));
            } };
